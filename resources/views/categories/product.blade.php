    ...
    <div style="display:flex;">
        <div class="productPicDiv">
            <img class="productImg"src="{{ $product->image }}"
                alt="ANASTASIA BEVERLY HILLS Waterproof Creme Liner Jet Black">
        </div>
...
<form action="{{ route('client.post.cart', $product->id) }}" method="POST">
    @csrf
    <input type="hidden" name="product_id" value="{{ $product->id }}" id="">
quantity: <input name="quantity" min="1" type="number" value="1" id="quantity" style="width:3.5em;">
<x-input-error :messages="$errors->get('quantity')" class="mt-2" />

<button  type="submit"class="addToCart p-2">Add to Cart</button> {{-- href="{{route("client.post.cart",$product->id)}}" --}}
</form>
<div><strong>description:<strong></div>
<p class="text-wrap" >{{ $product->desc }}</p>
